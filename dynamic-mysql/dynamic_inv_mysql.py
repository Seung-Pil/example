...
class MySQLInventory(object):
    ...
    def update_cache(self):
        ...
        for host in data:
            self.process_group(host['groups'])

            if 'hosts' in self.inventory[host['groups']]:
                self.inventory[host['groups']]['hosts'].append(host['host'])
            else:
                self.inventory[host['groups']].append(host['host'])
        ...
        gsql = """SELECT
               ...
               FROM childgroups
               LEFT JOIN `groups` `gparent` on `childgroups`.`parent_id` = `gparent`.`id`
               LEFT JOIN `groups` `gchild` on `childgroups`.`child_id` = `gchild`.`id`
